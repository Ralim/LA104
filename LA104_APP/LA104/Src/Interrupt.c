...
void SysTick_Handler(void)
{
    ...
    if(Counter % 2 == 0)                                  // 编码器采样周期 2mS
    {
        ...
        {
            KeyHold  = InputSt & KEYSBIT;                  // 识别按键保持状态 // Mask to get current IO status
            gKeyActv |= (~KeyHold & LastKey) << 4;         // 识别按键按下跳变 // Button Down
            gKeyActv |= (KeyHold & ~LastKey) << 8;         // 识别按键释放跳 // Button Up
            LastKey  = KeyHold;
